...
mcts1 = MCTS(game, system_nn, args)
fn_get_action_probabilities_for_system = lambda x: np.argmax(mcts1.getActionProb(x, temp=0))

human_vs_cpu = True
if human_vs_cpu:
    player2 = fn_human_player_policy(game)
else:
...
arena = Arena.Arena(fn_get_action_probabilities_for_system, player2, game, display=OthelloGame.display)
