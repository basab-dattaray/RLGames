...
class Board():
    ...
    def _increment_move(move, direction, n):
        move_copy = numpy.copy(move)
        move_to_check = list(map(sum, zip(move_copy, direction)))
        condition = all(map(lambda x: 0 <= x < n, move_to_check))
        while condition:
            move_copy = numpy.copy(move_to_check)
            yield move_copy
            move_to_check=list(map(sum,zip(move_copy,direction)))
            condition = all(map(lambda x: 0 <= x < n, move_to_check))
            pass
